...
import sys
class Dataset:
    def __init__(self, sketch_train_list, sketch_test_list, shape_list, num_views_shape=12, feaSize=4096, class_num=90, phase='train', normFlag=0):
        ...
        with open(sketch_test_list) as f:
            lines = f.readlines()
        self.sketch_test_data = [line.rstrip('\n') for line in lines]
    ...
    def loadAllData(self, phase):
        def loadShapeFeaAndLabel(pathSet, num_views, feaSize):
        ...
        def loadSketchFeaAndLabel(pathSet, feaSize):
            sampleNum = len(pathSet)
        ...
        self.sketchTestFeaset, self.sketchTestLabelset = loadSketchFeaAndLabel(self.sketch_test_data, self.feaSize)
        print("Loading time: {}".format(time.time() - start_time))

        print("Load sketch training features")
        start_time = time.time()
        self.sketchTrainFeaset, self.sketchTrainLabelset = loadSketchFeaAndLabel(self.sketch_train_data, self.feaSize)
        print("Loading time: {}".format(time.time() - start_time))

        print("Load shape features")
        start_time = time.time()
        self.shapeFeaset, self.shapeLabelset = loadShapeFeaAndLabel(self.shape_data, self.num_views_shape, self.feaSize)
        print("Loading time: {}".format(time.time() - start_time))
    ...
    def getLabel(self):
        ...
        for tmp_sketch in self.sketch_test_data:
            self.sketch_test_label.append(int(tmp_sketch.split(' ')[-1]))
